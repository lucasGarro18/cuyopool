...
var Ripples = function (el, options) {
	...
	if (this.interactive) {
		this.$el.on('mousemove.ripples', function(e) {
			...
			that.targetTilt.x = (that.mousePosition[0] - 0.5) * 3;   // CUYO: subtle tilt (was 30)
			that.targetTilt.y = (that.mousePosition[1] - 0.5) * -3;  // CUYO: subtle tilt (was -30)
...
Ripples.prototype = {
	...
	initShaders: function() {
		...
		this.updateProgram = createProgram(vertexShader, [
			...
			'void main() {',
				...
				'info.g += (average - info.r) * 2.0;',     // CUYO: stronger wave propagation
				'info.g *= 0.9985;',                        // CUYO: gentler velocity damping (was 0.993) — ondas viven 4× más
				'info.r += info.g;',
				'info.r *= 0.998;',                         // CUYO: minimal height damping (was 0.97) — pool-realistic decay
		...
		this.renderProgram = createProgram([
			...
			'void main() {',
				// ═══════════════════════════════════════════════════════════════
				// CUYO POOL — LIQUID GLASS shader (v11)
				// Only refractive displacement — no color tint, no caustics added,
				// no specular halos. Just water-glass distortion of the underlying image.
				// ═══════════════════════════════════════════════════════════════

				// 1. Sample ripple height map for surface normal
				...
				// 2. Gradient → surface normal (the direction water tilts at this point)
				...
				// 3. Compute refraction offset (where each pixel "bends" to)
				'vec2 offset = -normal.xz * perturbance;',

				// 4. Sample the underlying image at the REFRACTED coordinate
				// This is the entire "water effect" — pure displacement, like glass
				'vec4 finalColor = texture2D(samplerBackground, backgroundCoord + offset);',

				// 5. Add a very subtle bright glint on wave peaks (specular)
				// where the normal points upward — only visible on actual wave crests
				'float spec = pow(max(0.0, normal.y), 60.0);',
				'finalColor.rgb += spec * 0.10;',

				// 6. Ensure alpha is solid (no transparency artifacts)
				'finalColor.a = 1.0;',
	...
	startRain: function() {
		...
		this.rainInterval = setInterval(function() {
			...
			// CUYO v2: gotas más esporádicas y suaves — la imagen se nota mejor
			that.drop(x, y, that.dropRadius * (0.5 + Math.random() * 0.4), 0.008 + Math.random() * 0.012);
		}, 1400);  // CUYO v2: was 650ms → 1400ms (más de 2× más espaciado)
	},
